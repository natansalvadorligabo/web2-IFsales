...
as
    ...
    function rec_total_sales(p_by_category_id in number default null
                            ,p_by_month_number in number default null
                            ,p_by_region_id in number default null) return number;
...
-- PACKAGE BODY
create package body ifsales_pkg
as
--------------------------------------------------------------------------------------------------------
...
-- função para recuperar o total de vendas
    ...
    begin
        select nvl(sum(fun.funnel_product_qt * (fun.product_price - fun.funnel_discount)), 0)
        into v_total_sales
        from v_funnel  fun
        where (p_by_category_id is null or fun.product_category_id = p_by_category_id)
          and (p_by_region_id is null or fun.customer_region_id = p_by_region_id)
          and (p_by_month_number is null or extract(month from fun.funnel_paid_date) = p_by_month_number);
